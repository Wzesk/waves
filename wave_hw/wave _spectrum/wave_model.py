...
class wave_solver:
    ...
    def speed(self,wn,dr,eff_depth):
    ...
    def group_speed(self,wn,dr,eff_depth):
        #calculates group speed, including shallow water term
        wave_phase_speed = self.speed(wn,dr,eff_depth)
    ...
    def shoaling_coefficient(self, shoal_depth):
        ...
        deep_dr = self.dispersion(self.initial_depth)
        initial_group_speed = self.group_speed(self.initial_wavenumber, deep_dr,self.initial_depth) #deep group speed
        ...
        shoal_group_speed = self.group_speed(shoal_wave_number, shoal_dr,shoal_depth) #shoal group speed
        
        return np.sqrt(shoal_group_speed / initial_group_speed) #wave height ratio
        
    def refraction_coefficient(self,incedent_angle,shoaling_co):
        if incedent_angle == 0:
            return 1
        ...
        if -1 >= (np.sin(rad_incedent_angle) / shoaling_co) <= 1:#checking for acceptable input for arcsin
            print("angle: ",incedent_angle," shoaling: ",shoaling_co," resulted in an arcsin input error")
            return 1
        
        #calculate new refracted angle
        rad_refracted_angle = np.arcsin(np.sin(rad_incedent_angle) / shoaling_co)
        
        #new distance between rays
        ray_span_ratio = np.cos(rad_refracted_angle)/np.cos(rad_incedent_angle)
